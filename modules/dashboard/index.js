...
module.exports = client => {
    ...
    res.render("team", {
        ...
        app.get("/staff", checkAuth, async (req, res) => {
            ...
            if (!member) return res.redirect("/?error=true&message=" + encodeURIComponent("I don't have any Data of you, try asking k4itrun about it."));
        ...
        app.post("/createbot", checkAuth, async (req, res) => {
            try {
              ...
              if (bottype !== BotDir) return res.redirect("/createbot?error=true&message=" + encodeURIComponent("You are not allowed to create this Bot Type in that Ticket!"));
                ...
                //Check for ERRORS
                if (owner.length < 17 || owner.length > 19) return res.redirect("/createbot?error=true&message=" + encodeURIComponent("Invalid Owner ID, that would be a valid example: `313247783748501505`"))
                ...
                if (token.length != "MTA2NTY2MjQzMTc3MjI4NzEyNw.GD3r0n.JdinM-OLs5BEdSS8Srv0h8PMZyYDN2-hAOinJE".length) return res.redirect("/createbot?error=true&message=" + encodeURIComponent("INVALID TOKEN"))
                ...
                ticketChannel.send({
                    ...
                    embeds: [new Discord.MessageEmbed().setColor(client.config.color).addField("📯 Invite link: ", `> [Click here](https://discord.com/oauth2/authorize?client_id=${botuser.id}&scope=bot&permissions=8)`)
                        .addField("💛 Support us", `> **Please give us <#${mainconfig.FeedBackChannelID.toString()}> and stop at <#${mainconfig.DonationChannelID.toString()}> so that we can continue hosting Bots!**`).setTitle(`\`${botuser.tag}\` is online and ready to be used!`).setDescription(`<@${botuser.id}> is a **${BotType}** and got added to: <@${owner}> Wallet!\nTo get started Type: \`${prefix}help\``).setThumbnail(botuser.displayAvatarURL())
                        .addField("<:like:938142052087124008> Rate us on TRUSTPILOT", `> ***We would love it, if you could give us a __HONEST__ Rating on [Trustpilot](https://de.trustpilot.com/review/teamarcades.xyz)*** <3`)
                    ]
                }).catch(() => { });
         ...
         */
         try {
            ...
            const key = fs.readFileSync("/etc/letsencrypt/live/dash.teamarcades.xyz/privkey.pem", `utf8`);
            const cert = fs.readFileSync("/etc/letsencrypt/live/dash.teamarcades.xyz/fullchain.pem", `utf8`);
            const ca = fs.readFileSync("/etc/letsencrypt/live/dash.teamarcades.xyz/chain.pem", `utf8`);
