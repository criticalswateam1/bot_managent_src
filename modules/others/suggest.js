     ...
     let suggestReplace = await message.channel.send({
         embeds: [
             new Discord.MessageEmbed()
                 ...
                 .setFooter({ text: "Team Arcades", iconURL: 'https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png' })
                 .setColor(ee.color)
         ]
         
     }).catch(e => { console.log(String(e.stack).grey) });
     await suggestReplace.react(`<a:yes:1075484548005101691>`).then(() => { }).catch(e => { console.log(String(e.stack).grey) });
     await suggestReplace.react("❌").then(() => { }).catch(e => { console.log(String(e.stack).grey) })
    }
    })
}