...
module.exports = client => {
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if(interaction.customId == "closeticket"){
            interaction.reply({content: `$ **Now Closing the Ticket!**\n> Don't forget to leave <#${mainconfig.FeedBackChannelID.toString()}> and a review on our <https://de.trustpilot.com/review/teamarcades.xyz>`, ephemeral: true}).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
            setTimeout(async() => {
                if(channel) {
                    let userid = false;
                    try {
        ...
        if(interaction.customId == "TicketClaim") {
            ...
            if (member.roles.highest.rawPosition >= guild.roles.cache.get(Roles.NewSupporterRoleId).rawPosition) {
                if(channel.name.slice(0, 5).includes("💎") && member.roles.highest.rawPosition <= guild.roles.cache.get(Roles.FounderId).rawPosition - 1) {
                    return interaction.reply({ephemeral: true, content: ":x: **You are not allowed to claim, due to the fact its a k4itrun TICKET**"});
                }
                ...
                switch(menuIndex.type){
                    case "ClanBot" : {
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> ➡  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, 
                            embeds: [
                            ...
                            components: [
                                ...
                                if(interaction.message.channel.id == RecoverChannelId) msg.reply(`<a:yes:1075484548005101691>  **This is a REFUND ORDER**`).catch(() => {});
                            })
                    ...
                    case "WAITINGROOMBOT" : {
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> ➡  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, embeds: [new MessageEmbed().setColor("#6861fe")
                            .setTitle(`⏱ for Ordering a Waitingroom Bot! Send the Parameters!`)
                            .setDescription(ticket.message.replace("{user}", `${user}`)),                                new MessageEmbed().setColor("YELLOW").setTitle(`${emoji.loading} A Staff Member will claim the Ticket soon!`).setDescription(`**Dear ${user}!**\n> This Ticket will be claimed by a Staff Member as soon as possible!\n\n> *He/She/They will help you then!*\n\n> **Meanwhile, make sure to list us all Information needed!**`).setFooter("Thanks for choosing Team Arcades Development ✌️", "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png")],
                            components: [
                                new MessageActionRow().addComponents([
                                    new MessageButton().setStyle("PRIMARY").setLabel("Claim").setEmoji("📝").setCustomId("TicketClaim")
                            ...
                            interaction.editReply({content: `<#${ch.id}>`, embeds: [new MessageEmbed()
                                .setColor("#6861fe")
                                .setTitle(`⏱ Your Waitingroom Bot Order \`${ch.name}\` has been created!`)
                                .addField("💢 ATTENTION", `> *Please make sure, to fill out the Information the Bot sent you in the CHANNEL*`)
                                .addField("💬 The Channel:",`<#${ch.id}>`)], ephemeral: true}).then(msg=>{
                            }).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});            
                        }).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
                    ...
                    case "SECURITYBOT": {
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> ➡  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, embeds: [new MessageEmbed().setColor("#6861fe")
                            .setTitle(`⚔ Thanks for Ordering a Security Bot! Send the Parameters!`)
                            .setDescription(ticket.message.replace("{user}", `${user}`)),                                new MessageEmbed().setColor("YELLOW").setTitle(`${emoji.loading} A Staff Member will claim the Ticket soon!`).setDescription(`**Dear ${user}!**\n> This Ticket will be claimed by a Staff Member as soon as possible!\n\n> *He/She/They will help you then!*\n\n> **Meanwhile, make sure to list us all Information needed!**`).setFooter("Thanks for choosing Team Arcades Development ✌️", "https://cdn.discordapp.com/attachments/936372059305566219/964968487250436206/unknown.png")],
                            components: [
                                new MessageActionRow().addComponents([
                                    new MessageButton().setStyle("PRIMARY").setLabel("Claim").setEmoji("📝").setCustomId("TicketClaim")
                            ...
                            interaction.editReply({content: `<#${ch.id}>`, embeds: [new MessageEmbed()
                                .setColor("#6861fe")
                                .setTitle(`⚔ Your Security Bot Order \`${ch.name}\` has been created!`)
                                .addField("💢 ATTENTION", `> *Please make sure, to fill out the Information the Bot sent you in the CHANNEL*`)
                                .addField("💬 The Channel:",`<#${ch.id}>`)], ephemeral: true}).then(msg=>{
                                }).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)}); 
                        })
                    ...
                    case "SOURCECODE" : {
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> ➡  ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} ${emoji.arrow_left} `, embeds: [
