...
function GetBot(message, arg) {
    var errormessage = `❌ I Failed Finding That Bot...`;
    return new Promise(async (resolve, reject) => {
        ...
        if (!user && args[0] && args[0].length == 18) {
            ...
            return reject(`❌ **User Is Not A Discord Bot**...`)
        } else if (!user && args[0]) {
...
function theDB(client, guild) {
    ...
    **Please tell us all of those Informations!** | Otherwise, we can't create your Bot quickly!
    > 
    > ➡ **\` 1. \` What should be the Bot Name?**
    > *(The Bot's Username)*
    > 
    > ➡ **\` 2. \` What should be the PREFIX?**
    > *(The Letter(s) Infront of each Command, e.g: \`!\`)*
    > 
    > ➡ **\` 3. \` What should be the AVATAR?**
    > *(The Bot's Profile Picture)*
    > 
    > ➡ **\` 4. \` What should be the EMBED COLOR?**
    > *(The Color on the Left Side of Embeds)*
    > 
    > ➡ **\` 5. \` What should be the STATUS?**
    > *(e.g: \`!help | discord.gg/team-arcades-935157109761388554\`)*
    > 
    > ➡ **\` 6. \` What should be the STATUS TYPE?** 
    ...
    > [Patreon](https://www.patreon.com/teamarcades), PaySafe, Sofort, Boosting or \`INVITES\`
    ...
    *Someone will help you soon*`,
    ...
    client.setups.set(guild.id, {
        ...
        message: `Hey {user}! Thanks for opening a TICKET buying a <a:develoment:1075486210883068044> **SOURCE CODE**
    \`\`\`diff
    ...
    client.setups.set(guild.id, {
        ...
        guildid: guild.id,
        parentid: `${mainconfig.TicketCategorys.ModMailBotTicketsCategory}`,
        message: `Hey {user}! Thanks for opening a TICKET buying a <a:develoment:1075486210883068044> **CUSTOM BOT**
    \`\`\`diff
    - Please tell us what features the Bot should have!
    \`\`\`
    ➡ Name, Prefix, Avatar, Colorschema
    ➡ Features, If Music, Lavalink yes/no?
    ...
    **Please tell us all of those Informations!** | Otherwise, we can't create your Bot quickly!
    ...
    __**Payment Methods:**__
    > [Patreon](https://www.patreon.com/teamarcades), PaySafe Card, Sofort, Boosting once or \`INVITES\`
...
async function swap_pages2(client, message, embeds) {
    ...
    collector.on('collect', async b => {
        if (b?.user.id !== message.author.id)
            return b?.reply({ content: `❌ **Only The One Who Typed ${config.prefix}help Is Allowed To React!**`, ephemeral: true })
        //page forward
        if (b?.customId == "1") {
